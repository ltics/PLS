...
(defn pen-up
  ...
  [turtle]
  (dosync
    (alter turtle merge {:drawing false})))

(defn pen-down
  "Enable Drawing."
  [turtle]
  (dosync
    (alter turtle merge {:drawing true})))

(defn pen-color
  "Set pen color"
  [turtle color]
  (dosync
    (alter turtle merge {:color color})))
...
(defn right
  ...
  (let [angle (:angle @turtle)
    ...
    (dosync
      (alter turtle merge {:angle heading}))))
...
(defn- draw-line
  [turtle p1 p2]
  (doto
    (.getGraphics (:image @turtle))
    (.translate (/ (:width @turtle) 2)
                (/ (:width @turtle) 2))
    (.scale 1.0 -1.0)
...
(defn forward
  ...
  (let [p1 (:point @turtle)
        angle (:angle @turtle)
        p2 [(+ (first p1)
               (* (Math/sin (* angle deg)) steps))
            (+ (second p1)
               (* (Math/cos (* angle deg)) steps))]]
    (if (true? (:drawing @turtle))
      (draw-line turtle p1 p2))
    (dosync
      (alter turtle merge {:point p2}))))
...
(defn toward
  ...
  [turtle x y]
  (let [p (:point @turtle)
        polar (Math/atan2 (- (first p) x)
                          (- (second p) y))
        angle (mod (/ polar deg) 360)]
    (dosync
      (alter turtle merge {:angle angle}))))
...
(defn show
  ...
  (doto
    ...
    (.setSize (Dimension. (:width @turtle)
                          (:height @turtle)))
    (.show)))
